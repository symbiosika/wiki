#!/bin/sh
# Production entrypoint for the MCP server with optional Infisical secret
...
#
# The override file lets PR previews point at their OWN backend (OAUTH_ISSUER)
# even when Infisical injects a shared env. In prod the file is absent, so this
# is a no-op there.
#
