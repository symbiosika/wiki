...
import { pageUrl, annotatePageUrls, withPageUrls } from "./page-url.ts";
import { APP_BASE_URL } from "./config.ts";

const TENANT = "t-1";
const link = (pageId: string, anchor?: string) =>
  `${APP_BASE_URL}/tenant/${TENANT}/wiki/${pageId}${anchor ? `#${anchor}` : ""}`;

describe("pageUrl()", () => {
  test("builds the wiki SPA route", () => {
    expect(pageUrl(TENANT, "p1")).toBe(`${APP_BASE_URL}/tenant/t-1/wiki/p1`);
  });

  test("appends a heading anchor", () => {
    expect(pageUrl(TENANT, "p1", "urlaub")).toBe(
      `${APP_BASE_URL}/tenant/t-1/wiki/p1#urlaub`,
    );
  });
});
